...
def action_button(status: str, id_: str) -> rx.Component: #Assign/Reassign button based on status.
    # Use rx.cond for Reflex Var compatibility
    return rx.button(
...
def render_status(status: str) -> rx.Component: # create a function to render status badges
...
def assignment_row(item: Assignment) -> rx.Component: #create a table row for an assignment.
    return rx.table.row(
        ...
        rx.table.cell(
            rx.link(
                "View", href=f"/details/{item['id']}", color="blue", font_size="sm", font_weight="medium"
            )
        ),
...
def assignments_table() -> rx.Component:
    return (
        ...
        rx.hstack(
            rx.input(
            placeholder="Search by ID or Employee...", value=AssignmentState.search, on_change=AssignmentState.set_search, width="40%", size="2",
                ),
        rx.select(
            items=["All Statuses", "Unassigned", "In Review", "Completed"],
            value=AssignmentState.selected_status, on_change=AssignmentState.set_selected_status, width="20%", size="2",
                ),
                rx.button("Clear Filters", on_click=AssignmentState.reset_filters, size="2"),
                rx.button("Assign New Blueprint", color_scheme="blue", size="2", on_click=AssignmentState.open_new_assign_modal(),
                    ),
                    spacing="4", margin_y="1", width="100%",
                ),
                rx.box(
                rx.table.root(
                rx.table.header(
                ...
                rx.table.column_header_cell("Actions"),
                        ...
                        striped=True, highlight_on_hover=True, width="100%",
                    ),
                    width="100%",
                ),
                # Custom modal for reassignment (simulated with rx.cond and styled box)
                rx.cond(
                    AssignmentState.is_modal_open,
                    rx.box(
                        # Overlay
                        rx.box( position="fixed", top=0, left=0, width="100vw", height="100vh", bg="rgba(0,0,0,0.4)",z_index=1000,),
                        # Modal content
                        rx.center(
                            rx.card(
                            rx.vstack(
                            rx.heading(
                            rx.cond( AssignmentState.selected_case_id == "","Assign Reviewer","Reassign Reviewer"), size="4"),
                                ...
                                rx.select(
                                    ...
                                    rx.hstack(
                                        ...
                                        rx.button("OK", color_scheme="blue", on_click=AssignmentState.reassign_selected_case, is_disabled=AssignmentState.selected_reviewer == "",),
                                    ),
                                    spacing="4",
                                ),
                                width="350px", padding="2em", box_shadow="lg", bg="white", z_index=1001,
                            ),
                            position="fixed", top="50%",left="50%", transform="translate(-50%, -50%)", z_index=1001,),
                    ),
                    None
                ),
                # Modal for assigning a new blueprint
                rx.cond(
                    AssignmentState.is_new_assign_modal_open,
                    rx.box(
                        rx.box(
                            position="fixed", top=0, left=0, width="100vw", height="100vh", bg="rgba(0,0,0,0.4)", z_index=1000,
                        ),
                        rx.center(
                            rx.card(
                                rx.vstack(
                                    ...
                                    rx.hstack(
                                        ...
                                        rx.button(
                                            ...
                                            is_disabled=(AssignmentState.selected_new_blueprint == "") | (AssignmentState.selected_new_reviewer == ""),
                                        ),
                                    ),
                                    spacing="4",
                                ),
